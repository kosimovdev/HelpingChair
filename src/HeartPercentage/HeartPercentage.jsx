...
function HeartPercentage() {
  ...
  const [latestBpm, setLatestBpm] = useState(0); // oxirgi bpm interfeys uchun
  ...
  // ⚡ Asosiy interfeys uchun oxirgi BPM (1 soniya)
  useEffect(() => {
    ...
    const fetchLatest = async () => {
      try {
        ...
        const last = bpmData[bpmData.length - 1];
        setLatestBpm(last?.heartrate || 0);
      } catch (error) {
        console.error("Error fetching latest BPM:", error);
      }
    };

    fetchLatest(); // birinchi chaqirish
    const latestInterval = setInterval(fetchLatest, 1000);

    return () => clearInterval(latestInterval);
  }, [user_id]);

  // ⚡ LineChart uchun BPM ma’lumotlari (1 daqiqa)
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        ...
        if (Array.isArray(bpmData) && bpmData.length > 0) {
          const formatted = bpmData.map((item) => ({
            ...item,
            recorded_at: dayjs(item.recorded_at).format("YYYY-MM-DD HH:mm:ss"),
          }));
          setBpm(formatted);
        } else {
          setBpm([]);
        ...
        console.error("Error fetching BPM history:", err);
        setBpm([]);
      } finally {
        setLoading(false);
      }
    ...
    fetchChartData(); // birinchi chaqirish
    const chartInterval = setInterval(fetchChartData, 60000); // 1 daqiqa

    return () => clearInterval(chartInterval);
  }, [user_id]);

  // 🔔 Obstacle va FallModal logikasi
  useEffect(() => {
    if (!user_id) return;
